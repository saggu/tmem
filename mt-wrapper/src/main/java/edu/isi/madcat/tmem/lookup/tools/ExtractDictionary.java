...
public class ExtractDictionary {
  ...
  public ExtractDictionary(ParameterMap params) {
  ...
  public void process() {
    ...
    for (Biterm biterm : allBiterms) {
      String key = biterm.getSourceTermRaw();
    ...
    for (Map.Entry<String, List<Biterm>> sourceEntry : sourceToBiterm.entrySet()) {
      ...
      for (int i = 0; i < biterms.size(); i++) {
        ...
        String key = biterm.getTargetTermTok();
      ...
      for (Map.Entry<String, List<Integer>> targetEntry : targetToBiterm.entrySet()) {
        ...
        for (int i = 0; i < bitermIndexes.size(); i++) {
          ...
          if (indexToKeep == -1 || biterm.getTargetTermTok().length() > 0) {
            indexToKeep = index;
          }
        }
        if (indexToKeep != -1) {
  ...
  public static void main(String[] args) {
    ...
    ExtractDictionary processor = new ExtractDictionary(params);
    processor.process();
  }
}
