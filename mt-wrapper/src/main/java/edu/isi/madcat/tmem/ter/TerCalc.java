 ...
 * 
 * TERcalc.java v1 Matthew Snover (snover@cs.umd.edu)
 */
...
public class TerCalc {
  ...
  public static int numBeamCalls() {
    return NUM_BEAM_SEARCH_CALLS;
  }

  public static int numSegsScored() {
    return NUM_SEGMENTS_SCORED;
  }

  public static int numShiftsTried() {
  ...
  public static void setHypSpan(String span) {
    if (span != null && span.trim() != "") {
      ...
      for (int i = 0; i < spans.length; ++i) {
        ...
        hypSpans[i] = new TerIntPair(Integer.valueOf(s[0]), Integer.valueOf(s[1]));
      }
    }
  }
  ...
  public static void setRefLen(List<String> reflens) {
    String reflen = "";
  ...
  public static void setRefSpan(String span) {
    if (span != null && span.trim() != "") {
      ...
      for (int i = 0; i < spans.length; ++i) {
        String s[] = spans[i].split(":");
        refSpans[i] = new TerIntPair(Integer.valueOf(s[0]), Integer.valueOf(s[1]));
      }
    }
  }
  ...
  public static TerAlignment TER(Comparable<?>[] hyp, Comparable<?>[] ref, TerCost costfunc) {
    /* Calculates the TER score for the hyp/ref pair */
    Map<List<Comparable<?>>, Set<Integer>> rloc = BuildWordMatches(hyp, ref);
    ...
    while (true) {
      Object[] returns = CalcBestShift(cur, hyp, ref, rloc, cur_align, costfunc);
      if (returns == null) {
        break;
      }
  ...
  public static double terLowerBoundEdits(Comparable<?>[] hyp, Comparable<?>[] ref) {
    int maxLength = Math.max(hyp.length, ref.length);
    int maxMatches = 0;
    HashMap<Comparable<?>, Integer> hypCounts = new HashMap<Comparable<?>, Integer>();
    for (Comparable<?> word : hyp) {
      int count = hypCounts.containsKey(word) ? hypCounts.get(word) : 0;
      hypCounts.put(word, count + 1);
    }

    HashMap<Comparable<?>, Integer> refCounts = new HashMap<Comparable<?>, Integer>();
    for (Comparable<?> word : ref) {
      int count = refCounts.containsKey(word) ? refCounts.get(word) : 0;
      refCounts.put(word, count + 1);
    }

    for (Map.Entry<Comparable<?>, Integer> entry : hypCounts.entrySet()) {
      int hypCount = entry.getValue();
      int refCount = refCounts.containsKey(entry.getKey()) ? refCounts.get(entry.getKey()) : 0;
      maxMatches += Math.min(hypCount, refCount);
    }
    return (maxLength - maxMatches);
  }
  ...
  public static String[] tokenize(String s) {
    ...
    if (normalized) {
      ...
      s = s.replaceAll("'s$", " 's"); // handle possesives
      // s = s.replaceAll("\\b([^,\\. \\t]+)([\\.,])\\b", "$1 $2"); // tokenize period and comma at
      // end of word, unless there is an earlier period or comma
      // s = s.replaceAll("\\b([^,\\. \\t]+)([\\.,])\\s*$", "$1 $2"); // tokenize period and comma
      // at end of word, unless there is an earlier period or comma
      // s = s.replaceAll("\\b([^,\\. \\t]+)([\\.,])\\s+", "$1 $2 "); // tokenize period and comma
      // at end of word, unless there is an earlier period or comma
      s = s.replaceAll("\\s+([^,\\. \\t]+)([\\.,])\\s+", " $1 $2 "); // tokenize period and comma at
                                                                     // end of word, unless there is
                                                                     // an earlier period or comma

      // s = s.replaceAll("([^0-9])([\\.,])", "$1 $2 "); // tokenize period and comma unless
      // preceded by a digit
      // s = s.replaceAll("([\\.,])([^0-9])", " $1 $2"); // tokenize period and comma unless
      // followed by a digit
  ...
  private static Map<List<Comparable<?>>, Set<Integer>> BuildWordMatches(Comparable<?>[] hyp,
      Comparable<?>[] ref) {
    Set<Comparable<?>> hwhash = new HashSet<Comparable<?>>();
    ...
    HashMap<List<Comparable<?>>, Set<Integer>> to_return =
        new HashMap<List<Comparable<?>>, Set<Integer>>();
  ...
  private static Object[] CalcBestShift(Comparable<?>[] cur, Comparable<?>[] hyp,
      Comparable<?>[] ref, Map<List<Comparable<?>>, Set<Integer>> rloc, TerAlignment med_align,
      TerCost costfunc) {
    /*
     * return null if no good shift is found or return Object[ TERshift bestShift, TERalignment
     * cur_align ]
     */
    ...
    TerShift[][] poss_shifts =
        GatherAllPossShifts(cur, ref, rloc, med_align, herr, rerr, ralign, costfunc);
    ...
    for (int i = poss_shifts.length - 1; i >= 0; i--) {
      ...
      for (int s = 0; s < poss_shifts[i].length; s++) {
        ...
        double gain =
            (cur_best_align.numEdits + cur_best_shift_cost) - (curalign.numEdits + curshift.cost);

        if (DEBUG) {
          System.out.println("Gain for " + curshift + " is " + gain + ". (result: ["
              + TerAlignment.join(" ", shiftarr) + "]");
  ...
  private static TerShift[][] GatherAllPossShifts(Comparable<?>[] hyp, Comparable<?>[] ref,
      Map<List<Comparable<?>>, Set<Integer>> rloc, TerAlignment align, boolean[] herr,
      boolean[] rerr, int[] ralign, TerCost costfunc) {
    ...
    ArrayList<ArrayList<TerShift>> allshifts = new ArrayList<ArrayList<TerShift>>();

    for (int i = 0; i < MAX_SHIFT_SIZE + 1; i++)
      allshifts.add(new ArrayList<TerShift>());
    ...
    for (int start = 0; start < hyp.length; start++) {
      ...
      while (mti.hasNext() && (!ok)) {
        int moveto = mti.next().intValue();
        if ((start != ralign[moveto]) && (ralign[moveto] - start <= MAX_SHIFT_DIST)
      ...
      ok = true;
      for (int end = start; (ok && (end < hyp.length) && (end < start + MAX_SHIFT_SIZE)); end++) {
        ...
        while (movetoit.hasNext()) {
          int moveto = movetoit.next().intValue();
          if (!((ralign[moveto] != start) && ((ralign[moveto] < start) || (ralign[moveto] > end))
              && ((ralign[moveto] - start) <= MAX_SHIFT_DIST) && ((start - ralign[moveto]) <= MAX_SHIFT_DIST)))
            continue;
          ok = true;

          /*
           * check to see if there are any errors in either string (only move if this is the case!)
           */
          ...
          for (int roff = -1; roff <= (end - start); roff++) {
            ...
            if ((roff == -1) && (moveto == 0)) {
              if (DEBUG)
                System.out.println("Consider making " + start + "..." + end + " moveto: " + moveto
                    + " roff: " + roff + " ralign[mt+roff]: " + -1);

              topush = new TerShift(start, end, -1, -1);
            } else if ((start != ralign[moveto + roff])
                && ((roff == 0) || (ralign[moveto + roff] != ralign[moveto]))) {
              int newloc = ralign[moveto + roff];
              if (DEBUG)
                System.out.println("Consider making " + start + "..." + end + " moveto: " + moveto
                    + " roff: " + roff + " ralign[mt+roff]: " + newloc);
            ...
            if (topush != null) {
              ...
              allshifts.get(end - start).add(topush);
            }
          }
        }
      }
    ...
    for (int i = 0; i < to_return.length; i++) {
      to_return[i] = allshifts.get(i).toArray(new TerShift[0]);
    }
    return to_return;
  }
    ...
    for (j = 0; j <= hyp.length; j++) {
      ...
      for (i = first_good; i <= ref.length; i++) {
        ...
        if ((i < ref.length) && (j < hyp.length)) {
          if (refSpans == null || hypSpans == null || spanIntersection(refSpans[i], curHypSpans[j])) {
  ...
  private static Object[] PerformShift(Comparable<?>[] words, int start, int end, int newloc) {
    ...
    if (hypSpans != null)
      spans = new TerIntPair[hypSpans.length];
    if (DEBUG) {
      if (hypSpans != null) {
