...
public class KoreanTokenizer extends Tokenizer {
  ...
  private RegexReplacer replacer;
  ...
  private List<TokenInstance> getTokenInstances(List<TokenPointer> tokenPointers) {
    ...
    if (useSql) {
      ...
      List<String> ngramHashes = new ArrayList<String>();
      for (TokenPointer tp : tokenPointers) {
        String hashValue = QueryHash.getHashString(tp.getNgram());
        ngramHashes.add(hashValue);
        ptrMap.put(hashValue, tp);
      }

      List<String> querySets = SqlManager.getQuerySets(ngramHashes);
      SqlHandler sqlHandler = sqlManager.createHandler();
      for (String querySet : querySets) {
        ...
        for (int i = 0; i < results.size(); i++) {
          String key = results.get(i).get(0);
          int count = Integer.parseInt(results.get(i).get(1));
  ...
  private List<String> getOutputTokens(String rawToken) {
    String processedToken = rawToken.toLowerCase();
    processedToken = replacer.process(processedToken);
    processedToken = StringUtils.trim(processedToken);
    String[] tokenArray = processedToken.split("\\s+");
    List<String> tokens = new ArrayList<String>();
    for (String token : tokenArray) {
      List<String> splitWords = segmentWord(token);
